'''
Testing data and model scripts
'''
import os
import pandas as pd
import logging
import pytest
import joblib
# import yaml
import starter.ml.data
import starter.ml.model
import starter.train_model

# with open('config.yml') as f:
#     config = yaml.load(f)

@pytest.fixture
def test_import_data(path):
    '''
    test import_data 
    '''
    df = starter.ml.data.import_data(path)
    assert df.shape[0] > 0
    assert df.shape[1] > 0
    

def test_process_data(data):
    """
    test process_data function
    """
    cat_features = [
    "workclass",
    "education",
    "marital-status",
    "occupation",
    "relationship",
    "race",
    "sex",
    "native-country",
    ]

    X_test, y_test, _, _ = starter.ml.data.process_data(
        data, categorical_features=cat_features, label="salary", training=True)
       
    assert len(X_test) == len(y_test)

def test_columns_name(data):
    """
    test the columns dataset name
    """
    expected_columns = [
        "age",
        "workclass",
        "education",
        "education-num",
        "marital-status",
        "occupation",
        "relationship",
        "race",
        "sex",
        "hours-per-week",
        "native-country",
        "salary"
    ]

    obtained_columns = data.columns.values
    assert list(expected_columns) == list(obtained_columns)
    
def test_train_save_model():
    '''
    test train_models
    '''
    model = starter.train_model.train_save_model()

    # Models
    # 1. Check if the list is empty or not
    # 2. Check if all files exist
    try:
        # Getting the list of directories
        path = config['model']['filepath']
        dir_val = os.listdir(path)
        assert len(dir_val) > 0
        assert os.path.isfile('model/trainedmodel.joblib')
        assert os.path.isfile('models/encoder.joblib')
        assert os.path.isfile('models/lb.joblib')
        logging.info("Testing train_models: SUCCESS")
    except FileNotFoundError as err:
        logging.error("Testing train_models: Model files not found")
        raise err
